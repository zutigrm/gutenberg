...
function gutenberg_add_slug_to_fonts_registered_through_theme_json( $data ) {
	if ( empty( $data['settings'] ) || empty( $data['settings']['typography'] ) || empty( $data['settings']['typography']['fontFamilies'] ) ) {
		return $data;
	}

	foreach ( $data['settings']['typography']['fontFamilies'] as $index => $font_family ) {
		if ( ! isset( $font_family['slug'] ) ) {
			$font_family_name = isset( $font_family['font-family'] ) ? $font_family['font-family'] : $font_family['fontFamily'];

			$data['settings']['typography']['fontFamilies'][ $index ]['slug'] = WP_Webfonts::get_font_slug( $font_family_name );
		}
	}
...
function gutenberg_add_registered_webfonts_to_theme_json( $data ) {
	...
	foreach ( $to_add as $slug ) {
		if ( ! isset( $font_families_registered[ $slug ] ) ) {
			continue;
		}
